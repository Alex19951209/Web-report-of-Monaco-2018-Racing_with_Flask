...
class RecordData:
    ...
    timedelta | None
        ...
        if self.start >= self.stop:
            self.errors.append("Start time is after or equal to stop time.")
            return None
    ...
    tuple[list[RecordData], list[RecordData]]
        ...
        # Read the start data.
        records_dict: dict[str, RecordData] = self._read_start_stop(records_dict, folder, start_file, start=True)
        # Reading the stop data
        records_dict: dict[str, RecordData] = self._read_start_stop(records_dict, folder, stop_file, start=False)
        ...
        good_records_sorted = sorted(good_records, key=lambda r: r.duration, reverse=not asc)

        # Filter by driver name, if passed.
        if driver:
            good_records = [r for r in good_records if r.driver and driver.lower() in r.driver.lower()]
        return good_records_sorted, bad_records
...
def get_common_stats(order: str = "asc"):
    """Return sorted race results."""
    asc = order != "desc"
    record = RecordData()
    good, bad = record.build_report(asc=asc)
    return good, bad

def get_drivers(order: str = "asc"):
    """Return list of drivers with their abbreviations."""
    asc = order != "desc"
    record = RecordData()
    good, _ = record.build_report(asc=asc)
    drivers = [{"abbr": r.abbr, "name": r.driver, "team": r.team} for r in good]
    return sorted(drivers, key=lambda d: d["name"], reverse=not asc)

def get_driver_info(driver_id: str) -> dict | None:
    """Return detailed info about one driver by code (abbt)."""
    record = RecordData()
    good, bad = record.build_report()
    for r in good:
        if r.abbr == driver_id:
            return {
                "abbr": r.abbr,
